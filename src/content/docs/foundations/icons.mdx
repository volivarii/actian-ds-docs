...
import iconGlyphs from "../../../../vendor/components/dist/icons/icons.json";

export const glyphCount = Object.keys(iconGlyphs.icons).length;
...
{glyphCount} monochrome UI icons render below as live, themeable glyphs (they inherit text color via `currentColor`). The remaining connector and brand logos are multicolor and show as placeholder tiles pending a dedicated full-color asset pipeline.
